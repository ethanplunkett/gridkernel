...
kernelsmooth.grid<- function(x, kernel, kernelfun, max.r, kernel.dim=21, max.p.na=1, sample.points, weights,
                             no.match=0, use.old=FALSE, na.value = NA, ...	){
  
  # This function performs a kernelsmooth on a grid object. 
  #   For efficiency sake if kernelfun  is used and the cellsize of the grid 
  #   is such that the kernel created for that cellsize is greater than kernel.dim
  #   than the grid will be upscaled to coarser cells such that the kernel  dimension
  #   will be close to kernel.dim.  If the kernel argument is used instead than
  #   no upscaling will be done.
  #	 
  #
  # Arguments 
  ...
  if(!missing(kernelfun)){   # 
    ...
    # 
    args <-  c(list(max.r = max.r, cellsize  = x$cellsize), list(...))
    k <- do.call(kernelfun,  args = args)
    
  } else { # No kernelfun
    if(missing(kernel)) 
      stop("You need to specify either kernelfun or kernel")
    k <- kernel
  }
  
  print("Smoothing")
  x$m <- kernelsmooth(x$m, k, use.old = use.old) # Matrix method
